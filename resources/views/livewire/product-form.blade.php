    ...
    <div>
        <div class="mt-4">
            ...
            <div class="mt-2">
                ...
                <select wire:model="productStatus" id="productStatus" class="w-full mt-2 rounded-md form-input focus:border-indigo-600">
            ...
            <div class="flex flex-row mt-2">
                <div class="mr-2">
                    <img class="w-20 h-20" src="{{($productBanner ? $productBanner->temporaryUrl() : $product->banner ?? '' )}}" alt="">
                    <div wire:loading wire:target="productBanner">Uploading...</div>
                </div>
                <div>
                    <label class="text-gray-700" for="productBanner">Banner</label>
                    <input wire:model="productBanner" id="productBanner" class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="file">
                ...
                <div class="flex flex-row">
                    <div class="flex flex-row">
                        ...
                        @foreach ($productImages as $key => $image)
                            <div class="relative" x-data="{showDelIcon: false}" x-on:mouseover="showDelIcon = true" x-on:mouseout="showDelIcon = false" wire:key="{{$key}}">
                                <img class="w-20 h-20 mr-2" src="{{is_string($image) ? $image : $image->temporaryUrl()}}" alt="">
                                <span wire:click="deleteImage({{$key}})" x-show="showDelIcon" class="absolute w-5 h-5 cursor-pointer top-2 right-3">
                    ...
                    <div>
                        ...
                        <input multiple wire:model="newProductImages" id="newProductImages" class="w-full mt-2 rounded-md form-input focus:border-indigo-600" type="file">
